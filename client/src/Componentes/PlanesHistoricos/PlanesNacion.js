...
export class PlanesNacion extends Component {
  ...
  async eliminarPlanOperativo(){
    ...
    if(eliminar_plan_response !== 'err'){
      ...
      this.setState({
        ...
        mensaje: "Ha ocurrido un error al eliminar la Gaceta Oficial de la Nación"
      });
    }
  }
  ...
  async editarPlanOperativo(){
    if(this.verificarCamposModalEdicion()){
      // Verifica si se actualizó el archivo de la gaceta
      ...
      if(this.state.nuevo_fichero){
        ...
        if(subir_nuevo_fichero_response !== 'err'){
          ...
          if(actualizar_plan_response !== 'err'){
            this.setState({modal_editar_plan_operativo_abierto: false, modal_operacion_exitosa: true, mensaje: "Información de la Gaceta Oficial de la Nación actualizada exitosamente"}, async () => {
              this.obtenerPlanesOperativos();
            });
          }
          else{
            this.setState({modal_editar_plan_operativo_abierto: false, modal_operacion_fallida: true, mensaje: "Error al actualizar la información de la Gaceta Oficial de la Nación"});
          }
        }
  ...
  async crearPlanOperativo(){
    if(this.verificarCamposModalCreacion()){
      ...
      if(crear_plan_response !== 'err'){
        this.setState({modal_crear_plan_operativo_abierto: false, modal_operacion_exitosa: true, mensaje: "Gaceta Oficial de la Nación creado correctamente"}, async () => {
          this.obtenerPlanesOperativos();
        });
      }
      else{
        this.setState({modal_operacion_fallida: true, modal_crear_plan_operativo_abierto: false, mensaje: "Error agregando la Gaceta Oficial de la Nación"});
      }
    }
  }

  async componentDidMount(){
    document.title = 'SIGPRES CMB -Gacetas Oficiales de la Nación';
    this.obtenerPlanesOperativos();
  }
    ...
    let modal_crear_plan = 
      <Modal isOpen={this.state.modal_crear_plan_operativo_abierto} toggle={() => this.setState({modal_crear_plan_operativo_abierto: !this.state.modal_crear_plan_operativo_abierto})} size="md">
        <ModalHeader toggle={() => this.setState({modal_crear_plan_operativo_abierto: !this.state.modal_crear_plan_operativo_abierto})}>
          Crear nuevo Gaceta Oficial de la Nación
        </ModalHeader>
    ...
    let modal_editar_plan = 
      <Modal isOpen={this.state.modal_editar_plan_operativo_abierto} toggle={() => this.setState({modal_editar_plan_operativo_abierto: !this.state.modal_editar_plan_operativo_abierto})} size="md">
        <ModalHeader toggle={() => this.setState({modal_editar_plan_operativo_abierto: !this.state.modal_editar_plan_operativo_abierto})}>
          Editar Gaceta Oficial de la Nación
        </ModalHeader>
...
de la Nación</BreadcrumbItem>          
        ...
        <Row>
          ...
          <Col className="text-center" xs={12} sm={12} md={12} lg={12}>
            <img src={nacion} className="icono-titulo"/>    
            <h1 className="titulo-unidades-de-medida">Gestión de Gacetas Oficiales <br/> de la Nación</h1>
          </Col>
        ...
        {this.state.planes_operativos.length > 0 ?
          ...
          <Row className="justify-content-center">
            <Col xs={12} sm={12} md={6} lg={6} className="text-center"> 
              <h3>
                Aún no existen Gacetas Oficiales de la Nación 
                <br/> 
...
export default withContext(PlanesNacion);