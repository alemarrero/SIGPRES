  ...
  async eliminarPlanOperativo(){
    ...
    if(eliminar_plan_response !== 'err'){
      this.setState({
        ...
        mensaje: "Plan Operativo de la Alcaldía eliminado exitosamente"}, async () => {
      ...
      this.setState({
        ...
        mensaje: "Ha ocurrido un error al eliminar el Plan Operativo de la Alcaldía"
      });
    }
  }
      ...
      if(this.state.nuevo_fichero){
        ...
        if(subir_nuevo_fichero_response !== 'err'){
          ...
          if(actualizar_plan_response !== 'err'){
            this.setState({modal_editar_plan_operativo_abierto: false, modal_operacion_exitosa: true, mensaje: "Información del Plan Operativo de la Alcaldía actualizada exitosamente"}, async () => {
              this.obtenerPlanesOperativos();
            });
          }
          else{
            this.setState({modal_editar_plan_operativo_abierto: false, modal_operacion_fallida: true, mensaje: "Error al actualizar la información del Plan Operativo de la Alcaldía"});
          }
        }
  ...
  async crearPlanOperativo(){
    if(this.verificarCamposModalCreacion()){
      ...
      if(crear_plan_response !== 'err'){
        this.setState({modal_crear_plan_operativo_abierto: false, modal_operacion_exitosa: true, mensaje: "Plan Operativo de la Alcaldía creado correctamente"}, async () => {
          this.obtenerPlanesOperativos();
        });
      }
      else{
        this.setState({modal_operacion_fallida: true, modal_crear_plan_operativo_abierto: false, mensaje: "Error agregando el Plan Operativo de la Alcaldía"});
      }
    }
  }

  async componentDidMount(){
    document.title = 'SICMB - Planes Operativos de la Alcaldía';
    this.obtenerPlanesOperativos();
  }
    ...
    let modal_crear_plan = 
      <Modal isOpen={this.state.modal_crear_plan_operativo_abierto} toggle={() => this.setState({modal_crear_plan_operativo_abierto: !this.state.modal_crear_plan_operativo_abierto})} size="md">
        <ModalHeader toggle={() => this.setState({modal_crear_plan_operativo_abierto: !this.state.modal_crear_plan_operativo_abierto})}>
          Crear nuevo Plan Operativo de la Alcaldía
        </ModalHeader>
    ...
    let modal_editar_plan = 
      <Modal isOpen={this.state.modal_editar_plan_operativo_abierto} toggle={() => this.setState({modal_editar_plan_operativo_abierto: !this.state.modal_editar_plan_operativo_abierto})} size="md">
        <ModalHeader toggle={() => this.setState({modal_editar_plan_operativo_abierto: !this.state.modal_editar_plan_operativo_abierto})}>
          Editar Plan Operativo de la Alcaldía
        </ModalHeader>
      ...
      <Container fluid className="container-planes">
        ...
        <Row>
          ...
          <Col className="text-center" xs={12} sm={12} md={12} lg={12}>
            ...
            <h1 className="titulo-unidades-de-medida">Gestión de Planes Operativos <br/> de la Alcaldía</h1>
          </Col>
        ...
        {this.state.planes_operativos.length > 0 ?
          ...
          <Row className="justify-content-center">
            <Col xs={12} sm={12} md={6} lg={6} className="text-center"> 
              <h3>
                Aún no existen Planes Operativos de la Alcaldía 
                <br/> 
