...
export default class PresupuestoParticipativo extends Component {
  constructor(props){
    ...
    this.obtenerSugerenciasPresupuestoParticipativo = this.obtenerSugerenciasPresupuestoParticipativo.bind(this);
    this.eliminarSugerenciaPresupuestoParticipativo = this.eliminarSugerenciaPresupuestoParticipativo.bind(this);
  ...
  /**
   * Método utilizado para obtener las sugerencias de presupuesto participativo existente en el sistema
   *
   ...
   * información de las sugerencias de presupuesto participativo desde el servidor.
   */
  async obtenerSugerenciasPresupuestoParticipativo(){
    const sugerencias_request = await fetch('/api/presupuesto_participativo/obtener_sugerencias_presupuesto_participativo', {credentials: 'include'});
    ...
    if(sugerencias_response !== "err"){
      ...
      this.setState({modal_operacion_fallida: true, mensaje: "Error al obtener las sugerencias de presupuesto participativo"});
    }
  }
   ...
   * la información de las sugerencias de presupuesto participativo si la sugerencia se pudo eliminar 
   * correctamente.
   * @return muestra un modal de operación fallida si ocurrióun 
   * error al eliminar la sugerencia.
   */
  async eliminarSugerenciaPresupuestoParticipativo(id){
    ...
    let modal_confirmacion = 
      <Modal isOpen={this.state.modal_confirmacion_abierto} toggle={() => this.setState({modal_confirmacion_abierto: !this.state.modal_confirmacion_abierto})}>
        ...
        <ModalFooter>
          ...
          <Col xs={12} sm={12} md={6} lg={6} className="text-center">
            <Button color="danger" onClick={() => this.eliminarSugerenciaPresupuestoParticipativo(this.state.id)}>Eliminar</Button>
          </Col>
        </ModalFooter>
      </Modal>
    ;
    ...
    let modal_detalle_sugerencia = 
      <Modal isOpen={this.state.modal_detalle_sugerencia_abierto} toggle={() => this.setState({modal_detalle_sugerencia_abierto: !this.state.modal_detalle_sugerencia_abierto})}>
        ...
        <ModalFooter>
          ...
          <Col xs={12} sm={12} md={6} lg={6} className="text-center">
            <Button color="danger" onClick={() => this.setState({id: this.state.id, modal_detalle_sugerencia_abierto: false, modal_confirmacion_abierto: true})}>
              Eliminar
            </Button>
          </Col>
        </ModalFooter>
      ...
      <React.Fragment >
        ...
        <Row fluid={true}>
          ...
          <Col className="text-center" xs={12} sm={12} md={12} lg={12}>
            ...
            <h1>Sugerencias de Presupuesto Participativo</h1>
          </Col>
        </Row>
            ...
            <Table striped>
              ...
              {this.state.sugerencias.map((sugerencia, index) => {
                  ...
                  <tr key={`sugerencia_${index}`}>
                      ...
                      <th>{sugerencia.nombre}</th>
                      <th>{sugerencia.vision_comunidad.slice(0,120)}...</th>
                      <th>{sugerencia.identificador}</th>
                      <td>
                      <Button 
                          color="info" className="boton-ver"
                          onClick={() => this.props.history.push(`${this.props.location.pathname}/${sugerencia.id}`)}
                      >
