...
/**
 * Endpoint que permite incluir un nuevo plan de la Alcaldía en el sistema. Este endpoint recibe un multipart/form-data.
 * 
 ...
 */
router.post('/crear_plan_alcaldia', recibirArchivo, subirArchivo("planes_alcaldia"), function(req, res, next) {
  models.planes_alcaldia.create({
...
/**
 * Endpoint que se encarga de actualizar los datos de un plan de la Alcaldía.
 * 
 * Parámetros:
 * @param {Int} id id del plan de la Alcaldía dentro de la base de datos.
 * @param {String} nombre nombre con el que se identifica el plan de la Alcaldía
 * @param {String} periodo periodo al cual pertenece el plan de la Alcaldía
 * 
 * Respuestas:
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/actualizar_plan_alcaldia', autorizarAdministrador, function(req, res){
  models.planes_alcaldia.update({
    nombre: req.body.nombre,
    periodo: req.body.periodo,
  },
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/actualizar_archivo_plan_alcaldia', autorizarAdministrador, recibirArchivo, actualizarArchivo("planes_alcaldia"), function(req, res){
  models.planes_alcaldia.findOne({where: {id: req.body.id}})
  .then(plan => {
...
/**
 * Endpoint que se encarga de eliminar un plan de la Alcaldía. Adicionalmente, 
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/eliminar_plan_alcaldia', autorizarAdministrador, eliminarArchivo, function(req, res){
  if(req.archivo_borrado || !req.archivo_borrado){
    models.planes_alcaldia.destroy({where: {id: req.body.id}})
    .then(resultado => {
      if(resultado){
        res.status(200).json('ok');
      }
...
/**
 * Endpoint que habilita un plan de la Alcaldía en particular.
 * 
 * Parámetros:
 * @param {Int} id id del plan de la Alcaldía.
 * 
 * Respuestas:
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/habilitar_plan_alcaldia', autorizarAdministrador, function(req, res){
  models.planes_alcaldia.update({
    habilitado: true 
  }, {where: {id: req.body.id}})
  .then(x => {
...
/**
 * Endpoint que deshabilita un plan de la Alcaldía en particular.
 * 
 * Parámetros:
 * @param {Int} id id del plan de la Alcaldía.
 * 
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/deshabilitar_plan_alcaldia', autorizarAdministrador, function(req, res){
  models.planes_alcaldia.update({
    habilitado: false 
  }, {where: {id: req.body.id}})
  .then(x => {
...
/**
 * Endpoint que devuelve todos los planes de la Alcaldía que se encuentran en la base de datos.
 * 
 /**
 * Endpoint que habilita un plan de la Alcaldía en particular.
 * 
 ...
 * 
 * Respuestas:
 * @return estado 200 y un arreglo de objetos que representan los planes de la Alcaldía existentes
