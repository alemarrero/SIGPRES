...
/**
 * Endpoint que permite incluir un nuevo plan de la nación en el sistema. Este endpoint recibe un multipart/form-data.
 * 
 ...
 */
router.post('/crear_plan_nacion', recibirArchivo, subirArchivo("planes_nacion"), function(req, res, next) {
  models.planes_nacion.create({
 ...
 * @param {String} nombre nombre con el que se identifica el plan de la nación
 * @param {String} periodo periodo al cual pertenece el plan de la nación
 * 
 * Respuestas:
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/actualizar_plan_nacion', autorizarAdministrador, function(req, res){
  models.planes_nacion.update({
    nombre: req.body.nombre,
    periodo: req.body.periodo,
  },
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/eliminar_plan_nacion', autorizarAdministrador, eliminarArchivo, function(req, res){
  if(req.archivo_borrado || !req.archivo_borrado){
    models.planes_nacion.destroy({where: {id: req.body.id}})
    .then(resultado => {
      if(resultado){
        res.status(200).json('ok');
      }
...
/**
 * Endpoint que habilita un plan de la nación en particular.
 * 
 * Parámetros:
 * @param {Int} id id del plan de la nación.
 * 
 * Respuestas:
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/habilitar_plan_nacion', autorizarAdministrador, function(req, res){
  models.planes_nacion.update({
    habilitado: true 
  }, {where: {id: req.body.id}})
  .then(x => {
...
/**
 * Endpoint que deshabilita un plan de la nación en particular.
 * 
 * Parámetros:
 * @param {Int} id id del plan de la nación.
 * 
 ...
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.post('/deshabilitar_plan_nacion', autorizarAdministrador, function(req, res){
  models.planes_nacion.update({
    habilitado: false 
  }, {where: {id: req.body.id}})
  .then(x => {
...
/**
 * Endpoint que devuelve todos los planes de la nación que se encuentran en la base de datos.
 * 
 /**
 * Endpoint que habilita un plan de la nación en particular.
 * 
 ...
 * 
 * Respuestas:
 * @return estado 200 y un arreglo de objetos que representan los planes de la nación existentes
 * dentro de la base de datos.
 * @return estado 500 y 'err' si ocurrió algún error en el servidor.
 * @return estado 401 y 'err' si el usuario no se encuentra autenticado.
 * @return estado 403 y 'err' si el usuario no posee el rol necesario.
 */
router.get('/obtener_planes_nacion', autorizarAdministrador, function(req, res){
  models.planes_nacion.findAll()
  .then( resultado => {
